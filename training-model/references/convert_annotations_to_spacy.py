...
from lines import preprocess_merge_lines
...
def references_to_spacy_doc(
    references, style: str, fname: Optional[str] = None, part=0, merge_lines=MERGE_LINES
) -> Optional[Doc]:
    """
    Args:
        references - bibliography (list of references) generated by the csl-js processor
        style - style the references are generated for
        fname - croccref citeproc jsonl file bibliography has been generated from
        part - a part of larger bibliography rendered by the CSL processor from a single file downloaded from CrossRef (it contains 100 references)
        merge_lines - enable preprocessing as described below:
    ...
    augmenter = None
    if merge_lines:
        norm_references = (preprocess_merge_lines(ref) for ref in references)
